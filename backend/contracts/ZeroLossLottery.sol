...
contract ZeroLossLottery is Ownable {
    // State variables
    IERC20 public coin;
    uint256 public ticketPrice = 1 * 10**18; // 1 COIN (with 18 decimals)
    ...
    // Constructor now accepts the address of actual COIN on Sepolia
    ...
    function buyTickets(uint256 numTickets) external {
        ...
        require(coin.balanceOf(msg.sender) >= totalCost, "Insufficient COIN balance");
        
        // Transfer COIN from user to contract
        bool success = coin.transferFrom(msg.sender, address(this), totalCost);
    ...
    function addInterest(uint256 percentage) external onlyOwner {
        ...
        require(coin.balanceOf(msg.sender) >= interestAmount, "Insufficient admin COIN balance");
        
        // Transfer interest from admin to contract
        bool success = coin.transferFrom(msg.sender, address(this), interestAmount);
    ...
    function claimFunds() external {
        ...
        bool refundSuccess = coin.transfer(msg.sender, refundAmount);
        require(refundSuccess, "Refund transfer failed");
        
        if (prizeAmount > 0) {
            bool prizeSuccess = coin.transfer(msg.sender, prizeAmount);
    ...
    function claimPlatformFee() external onlyOwner {
        ...
        bool success = coin.transfer(owner(), platformFee);
